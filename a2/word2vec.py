...
def naiveSoftmaxLossAndGradient(
    ...
    outside_softmax_vec = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(outside_softmax_vec[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.sum(outside_softmax_vec.reshape(outside_softmax_vec.shape[0], 1)*outsideVectors, axis=0)
    gradOutsideVecs = outside_softmax_vec.reshape(outside_softmax_vec.shape[0], 1)*centerWordVec
    gradOutsideVecs[outsideWordIdx] = (outside_softmax_vec[outsideWordIdx]-1)*centerWordVec
...
def negSamplingLossAndGradient(
    ...
    U = np.take(outsideVectors, indices, axis=0)
    UV = np.dot(U, centerWordVec)
    UV[1:] = UV[1:]*-1
    loss = -1*np.sum(np.log(sigmoid(UV)))
    U[1:] = U[1:]*-1
    gradCenterVec = np.sum(U * (sigmoid(UV)-1).reshape(U.shape[0], 1), 0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    for i, ind in enumerate(indices):
        if ind == outsideWordIdx:
            gradOutsideVecs[ind] += (sigmoid(UV[i])-1)*centerWordVec
        else:
            gradOutsideVecs[ind] += -(sigmoid(UV[i])-1)*centerWordVec
    ### END YOUR CODE
